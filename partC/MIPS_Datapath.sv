...
//   BranchNotEqual  : 0 for beq, 1 for bne
...
module MIPS_Datapath (
    ...
    logic [4:0]  write_reg;
    logic        pc_src;
    logic	 ble_taken; // Part C Modification
    ...
    assign write_data =
        (instr[31:26] == 6'b001000 ||   // addi
         instr[31:26] == 6'b100011 ||   // lw
         instr[31:26] == 6'b000010 ||   // j
         instr[31:26] == 6'b001010) ?   // slti
         32'b0 : rd2;
    ...
    // Part C Modification
    // BLE branch decision
    // Takes the branch when rs <= rt using signed comparison
    // --------------------------------------------------------
    assign ble_taken = (instr[31:26] == 6'b111011) && ($signed(rd1) <= $signed(rd2));

    // --------------------------------------------------------
    // Part C Modification
    // Added BLE support to the branch decision logic
    // BEQ: branch when zero = 1
    // BNE: branch when zero = 0
    // BLE: branch when rs <= rt
    // --------------------------------------------------------
    assign pc_src = ble_taken |
                    ((instr[31:26] != 6'b111011) & Branch & (BranchNotEqual ? ~zero : zero));
